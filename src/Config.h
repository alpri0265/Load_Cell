...
constexpr float   JUMP_RESET_THRESHOLD_KG = 0.3f; // якщо новий відлік відхиляється від поточного
                                                   // показника більше ніж на це значення - це реальна
                                                   // зміна навантаження (не шум), фільтр миттєво
                                                   // "перескакує" на нове значення замість повільного
                                                   // усереднення протягом WEIGHT_FILTER_SIZE відліків.
                                                   // Занадто мале значення -> тремтіння показника від
                                                   // шуму датчика; занадто велике -> дрібні реальні зміни
                                                   // ваги знову йдуть повільним усередненням. Підберіть
                                                   // під реальний шум вашого датчика (дивіться покази в
                                                   // стані спокою без навантаження).
